...
nested_dict = {
    "scale_all": ["mu_s","phi_s_raw","kappa_s","mu_f","phi_f_raw","kappa_f"],
    "phi_raw_scale": ["phi_s_raw","phi_f_raw"],
    "kappa_scale": ["kappa_s","kappa_f"]
}
...
def observed_data(idata):
    obs_name = list(idata.observed_data.data_vars)[0]
...
def refit_case_deletion_grid(y, t_list, priors, time_series, prefix = "case_deletion"):
    ...
    for t in t_list:
        ...
        out[key] = idata
        az.to_json(idata, f"{time_series}_{prefix}_t{t}_deletion.json")
    
    return out

def refit_obs_shift_grid(y, time_series, t_list, delta_list, priors=BASE_PRIORS, prefix="obs_shift"):
    out = {}
    for t in t_list:
...
def refit_prior_perturbation_grid(y, etas, time_series, scheme="scale_all", prefix="prior_pert"):
    ...
    for eta in etas:
        ...
        az.to_json(idata, f"{time_series}_{prefix}_{scheme}_eta{eta:+.3f}.json")
    return out
