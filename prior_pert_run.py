...
idata_data = [['LYC_Daily_idata.json', 'LYC_Daily.csv'], ['USDJPY_idata.json', 'USDJPY.csv'], ['GBPJPY_data.json', 'GBPJPY.csv']]
...
for file in idata_data:
    ...
    print(f"Iterating through {file}")
    ...
    case_deletion_runs = ppf.refit_case_deletion_grid(y, t_list, BASE_PRIORS)
    
    print("Implementing observation pertubations...")
    obs_shift_runs = ppf.refit_obs_shift_grid(y, t_list=t_list, delta_list=delta_list)
    
    print("Implementing prior pertubations...")
    etas = [-0.5, -0.25, 0.0, 0.25, 0.5]
    prior_runs_phi = ppf.refit_prior_perturbation_grid(y, etas, scheme="phi_raw_scale", prefix="prior_pert")
